...
## Chap 4: 数据语义学
编译器对于成员函数的解析是整个类定义完成之后才做的， 而成员函数的参数类型是最近一次遇到时决定的。 例如下面这个例子中， func 看到的 mytype 就是全局的 string,而不是后于它定义的 int：
```c++
#include <string>
using namespace std;
...
```
类中的这种类型定义语句一定要放在类内的开始位置来定义， 成员函数遇到的时候用**最近碰到的类型原则**就不会出现解析错误。  
### 进程内存的空间布局
可执行文件加载到内存中就编程了一个进程， 虚拟地址空间包括下面的部分：
1. stack: 局部变量等放在这里
2. heap: new/malloc 等动态申请的放在这里
3. BSS 段： 未初始化、初始化为0 的全局变量（其实是包含在 data 段的）
4. data 段： 已经初始化的全局变量
5. 代码段

在 unix 下可以使用 nm 列出可执行文件中全局变量等存放的位置。  

### 数据成员
**在内存中的布局**：
1. 静态成员变量是跟着类走的， 不占用对象的内存空间（实际上是存在整个程序的数据段中）
2. 同一个 access section 中的变量按照定义中的顺序存储（不同 section 的先后是未定义的）
3. 对象占用的内存是一块连续内存
4. 编译器会自动做边界调整， 使得 sizeof 出来的结果为 4或者 8 的整数倍(`#pragma pack(1)`可以取消)

**数据成员的存取**：
1. 静态变量可以看成是一个全局的变量， 只是它只在类的空间内可见
   1. 使用类名、变量名、指针访问数据的操作符分别为 `::`/`.`/`->`
2. 非静态成员存放在对象中， 要通过对象来存取
3. 访问一个非静态成员变量实质上就是对象的首地址 + 偏移量，在有继承关系的情况下， 这种偏移量也是提前计算好的
   1. 虚基类的情况例外， 需要一些间接手段， 比普通的变量有一些效率损失

### 继承下的成员布局
<div align=center><img src="https://i.imgur.com/TJwPKA9.png"/></div>
<div align=center><img src="https://i.imgur.com/sYUQZdH.png"/></div>

对于继承体系中的类， 一个子类对象的成员是自己的成员和父类成员之和， 从偏移值上看， 父类成员是先出现的， 然后是子类成员（虚基类除外）。**至于继承时父类的填充（边界调整）是否保留， 不同厂商的编译器实现的细节可能不同**。 在这种情况下， 可能是 Derived 占用了 Base 中原本用于调整的部分，这时候用 memcpy 从 base 往 Derived 拷贝其父类数据可能覆盖那部分的数据。  

...
同样，堆栈也可以有数组实现和链表实现。
- 数组实现：用一个[一维数组]()和一个记录[栈顶元素位置的变量]()组成,也可以用一个array存两个stack，一个向上增长，一个向下增长，两个Tag相遇就是堆栈满了<br>
...
一个好的散列函数应该拥有的特点:
* 计算简单,以方便提高转换速度
* 关键词对应的地址空间分布均匀, <font color = red>尽量减少冲突</font>

数字关键词的散列函数构造:
1. 直接定址法: $h(key) = a \times key + b$
2. 除留余数法: $h(key) = key\ mod\  p$,一般p取值为为素数
3. 数字分析法: 分析关键字在各位上的变化情况,取比较随机的未作为散列地址<br>
    例如, 手机号码11位一般key用最后的四位, 和身份证号码 
    ![id hash](figure/ZJU11.1.png)
4. 折叠法: 把关键词分割成位数相同的几个部分,然后叠加<br>
    ![hash new](figure/ZJU11.2.png)
5. 平方取中法:<br> 
    ![hash new](figure/ZJU11.3.png)

字符关键词的散列函数构造:
1. 一个简单的散列函数: ASCII码加法和再用mod(会有冲突和聚集)
2. 简单的改进: 前三个字符位移法(依然会有冲突和空间浪费的情况)
3. 好的散列函数: 移位法<br>
    $h(key) = (\sum _{i=0}^{n-1} key[n-i-1]\times 32^i) \text{ mod } TableSize$ 
    选用32是因为可以用左移五位的做法实现
...
开放地址法: 一旦产生了冲突,就用某种规则寻找另一个位置(线性探测$(d_i + i)$, 平方探测$(d_i \pm i^2)$, 双散列$(d_i = i*h_2(key))$)
#### 线性探测法
用一个增量序列1,2,3...,TableSize-1 循环试探下一个存储地址, [会有聚集现象,在冲突处的值越来越多]().<br>
散列表查找性能分析:
* 成功平均查找长度(ASLs)
* 不成功平均查找长度(ASLu)<br>
![linear hash find](figure/ZJU11.4.png)

#### 平方探测法 
使用一个增量序列$1,-1,2^2,-2^2,3^2,-3^2...,q^2,-q^2$,并且$q\leq[TableSize/2]$循环试探下一个存储地址. 可能存在表中依然有空间,但是平方探测找不到的情况.改进: 如果散列表的长度是某个4k+3形式的质数时, 平方探测就可以探查到整个散列表空间.

#### 双散列表
对于任意一个key, $h_2(key) \neq 0$(否则就没有偏移),一般认为设计成 $h_2(key) = p - (key \ mod \ p)$ 就具有良好的效果, 其中 p < TableSize, p tablesize都是质数.

#### 再散列
* 当散列表元素太大(装填因子$\alpha$ 太大)时,查找效率会下降
* 散列表扩大时, 所有元素都要重新计算放置到新的表中

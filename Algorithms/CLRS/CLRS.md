...
#### 应用动态规划
* **最优括号方案的结构特点**：一个非平凡的矩阵链乘法问题实例的任何解都需要划分链，而任何最优解都是由子问题实例的最优解构成的。因此，我们可以将问题划分为两个子问题 ($A_i A_{i+1}\cdots A_k$ 和 $A_{k+1} A_{K+2}\cdots A{j}$ 的最优括号化问题），求出子问题实例的最优解，然后将子问题的最优解组合起来。
* **一个递归求解的方案**：
  * 令 $m[i,j]$ 表示计算 $A_{i,j}$ 需要的最小的乘法次数， 原问题的解就是 $A[1,n]$
  * 假设我们找到了k, 那么 $m[i,j] = m[i,k] + m[k+1,j] + p_{i-1} p_k p_j$
* **计算最优代价**: k 有 j-i 种取值， 我们挨个查找即可 
    $$
    m[i,j] = \begin{cases}
                0 & \text{if}\ i = j\\
                \min_{k=i}^{j-1} m[i,k] + m[k+1,j] + p_{i-1} p_k p_j & \text{if}\ i < j
            \end{cases}
    $$
* 构造最优解： 使用表 m 记录次数， 使用表 s 记录 k 的值，即可从 s 中构造出来最优解
    <div align=center><img src="https://i.imgur.com/vUHHr21.png" width="80%"/></div>

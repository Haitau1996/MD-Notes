...
### 中介者模式
#### 动机
* 在软件构建过程中，经常会出现多个对象互相关联交互的情况，对象之间常常会维持一种**复杂的引用关系**，如果遇到—些需求的更改，这种直接的引用关系将面临不断的变化。
* 在这种情况下，我们可使用—个“中介对象”来管理对象间的关联关系，避免相互交互的对象之间的紧耦合引用关系，从而更好地抵御变化。

例如如果我们有一个界面, 上面有很多控件, 而其背后是很多 data model. 我们希望更改界面的时候, 背后的 data model 也发生变化; 同时 data model 的变化也应该显示在界面上: 这导致双向或者是直接的依赖关系.

#### 定义
> 用一个中介对象来封装(**封装变化**)一系列的对象交互。中介者使各对象不需要显式的相互引用（编译时依赖➔ 运行时依赖），从而使其耦合松散（管理变化），而且可以**独立地**改变它们之间的交互。

#### 结构
<div align=center><img src="https://i.imgur.com/sUgkeoe.png"/></div>

在现实中 ConcreteColleague1 和 ConcreteColleageu2 之间可能不一定是继承自同一个类, 这个结果就是使得两者不直接依赖.
<div align=center><img src="https://i.imgur.com/09WD7y5.png"/></div>

这个结构有点像门面模式, 门面模式是对系统内和系统外的对象之间的解耦,而中介者是对内各个对象之间的解耦, 它是依赖倒置原则在多个对象之间的一种拓展实现.
...
* Facade模式是解耦系统间（单向）的对象关联关系； Mediator 模式是解耦系统内各个对象之间（双向）的关联关系。

## 状态变化模式
在组件构建过程中，某些对象的状态经常面临变化，如何对这些变化进行有效的管理？同时又维持高层模块的稳定？”状态变化”模式为这—问题提供了一种解决方案。
* State
* Memento
...
例如在我们实现的一个[平凡的网络状态操作](code/NetworkStateTrivalImpl.cpp),在里面有大量的 If-Else 块去判断同样的操作在不同状态下的行为. 如果以后有其他的状态, 如 NetWork_wait, 那么所有的操作中都要添加 else if 块, 违背了开放封闭原则.   
[好的实现](code/NetworkStateAfterRefactoring.cpp)可以提出一个抽象基类,状态从枚举就变成了一个状态对象. 将所有和状态有关的操作变成状态对象的行为(虚成员函数, 虚函数的本质其实是运行时的 if-else), 然后包含一个状态对象的指针. 由于性能原因只需要一个状态对象, 其中也可以应用单例模式.

#### 定义
> 允许—个对象在其内部状态改变时改变它的行为。从而使对象看起来似乎修改了其行为。

#### 结构
<div align=center><img src="https://i.imgur.com/cCpSk9r.png"/></div>

#### 要点总结
* 将所有与一个特定状态相关的行为放入一个 State 的子类对象中, 在对象状态切换时, 切换相应的对象并且保持 State 的接口, 这样实现了具体操作与状态转换之间的解耦
* 为不同状态引入不同的对象使得状态的转换变得更加明确, 并且可以保证不会出现状态不一致的情况, 因为转换是原子性的(要么彻底转换, 要么不转换)
* 如果 State 对象没有实例变量, 上下文可以共享同一个 State 对象, 从而节省开销

这个和策略模式很像, 重构前都有很多 if-else 语句块, 我们使用多态的方式消除了这些语句. 

### 备忘录模式
#### 动机
* 在软件构建过程中，某些对象的状态在转换过程中，可能由于某种需要，要求程序能够回溯到对象之前处于某个点时的状态。如果使用—些公有接口来让其他对象得到对象的状态，便会暴露对象的细节实现。
* 如何实现对象状态的良好保存与恢复？但同时又不会因此而破坏对象本身的封装性。

#### 定义
> 在不破坏封装性的前提下，拥获一个对象的内部状态，并在该对象之外保存这个状态。这样以后就可以将该对象恢复到原先保存的状态。

#### 结构
<div align=center><img src="https://i.imgur.com/fkpbWaF.png"/></div>

#### 要点总结
* 备忘录(Memento) 存储原发器(Originator) 对象的[内部状态](code/memento.cpp)，在需要时恢复原发器状态。
* Memento 核心是信息隐藏，即 Originator 需要向外接隐藏信息，保持其封装性。但同时又需要将状态持到外界
* 由于现代语言运行时如(C# 、Java等)都具有相当的对象序列化支持, 因此往往采用较高、又较容易正确实现的序列化方案来实现 Memento 模式。

## 数据结构模式
常常有—些组件在内部具有特定的数据结构，如果让客户程序依赖这些特定的数据结构，将极大地破坏组件的复用。这时候，将这些**特定数据结构封装在内部，在外部提供统一的接口**，来实现与特定数据结构无关的访问，是—种行之有效的解决方案。
* Composite
* Iterator
...
#### 定义
> 将对象组合成**树形结构**以表示“部分－整体”的层次结构。_Composite_ 使得用户**对单个对象和组合对象的使用具有一致性（稳定）**。

在一个[典型的实现](code/composite.cpp)中, 我们定义了一个虚基类 _Component_, 然后派生出树节点 _Composite_ 和叶节点 _Leaf_, 根据特性覆写相应的纯虚函数(里面有一种多态的递归, 针对所有的节点都做处理). 实际上做的事情就是**把这种树形的数据结构封装在类的内部, 而不是暴露给外界**.

#### 结构
需要注意的是 add 和 remove 都没有放在 Leaf 中是有一点尴尬, 重点是 Operation 处理当前节点后处理子节点, 有多态的递归调用.
<div align=center><img src="https://i.imgur.com/M50H3GD.png"/></div>

#### 要点总结
* 采用树形结构实现普遍存在的对象容器，从而将“一对多”的关系转化为"一对一"的关系吏得宝代码可以一致地(复用, 如果不这么做, 在 Invoke 的时候需要判断处理的是不是叶子节点, 据此做相应的处理)处理对象和对象容器，无需关心处理的是单个的对象，还是组合的对象容器。
* 将客代码与复杂的对象容器结构解耦是 _Composite_ 的核心思想，之后客户代码将与纯粹的抽象接口(非对象容器的内部实现结构)发生依赖, 从而可以应对变化
* 具体实现的时候可能父子可以反向追溯, 有频繁的遍历需求可以通过缓存来改善效率

### 迭代器
#### 动机
在软件构建过程中，集合对象内部结构常常变化各异。但对千这些集合对象，我们希望在不暴露其内部结构的同时，可以让外部客户代码透明地访问其中包含的元素；同时这种"透明遍历"也为"同一种算法在多种集合对象上进行操作"提供了可能。
#### 定义
> 提供一种顺序的方法访问一个聚合对象中的各个元素, 而又不暴露（隔离变化）该对象的内部表示。

GoF 是使用面向对象的方式来[实现](code/Iterator.cpp), 就是用虚函数来实现各种操作(`First()`,`Next()`,`IsDone()`,`GetCurrent`), 这是有性能成本的. 使用模板的话, 使用编译时多态有更好的性能, 同时也支持更多的可能性, 如往前走, 往后走, 走 n 步, 而没有第三方的类库用 OO 的方式实现这么多种操作: 因此有了泛型编程的迭代器后就没有人在 C++ 中使用 OO 的方式实现迭代器了. 
#### 结构
<div align=center><img src="https://i.imgur.com/rmJCWPK.png"/></div>

#### 要点总结
* 迭代抽象：访问一个聚合对象的内容而无需暴露它的内部表示(链表实现,vector实现, 堆实现等等)。
* 迭代多态：为遍历不同的集合结构提供一个统—的接口，从而支持同样的算法在不同的集合结构上进行操作。
...
#### 定义
> 使多个对象都有机会处理请求，从而避免请求的发送者和接收者之间的耦合关系。将这些对象**连成一条链，并沿着这条链传递请求**，直到有一个对象处理它为止。

这个链实际[实现](code/ChainofResposibility.cpp)是一个链表, 这时候处理请求的类派生自基类 _ChainHandle_,它里面包含了一个多态指针, 并且可以设置下一个 _Handler_. 处理的基本逻辑就是, 如果可以处理, 就在本类中处理, 否则将请求发送给下一个 _Handler_. 然后具体的 _Handler_ 覆写相应的 `canHandleRequest()`/`processRequest()`.

#### 结构
<div align=center><img src="https://i.imgur.com/dvECtOU.png"/></div>

#### 要点总结
* 这个类现在用的其实不多, 应用之后都西昂的职责分派更具有灵活性, 我们可以在运行时添加/修改 请求的处理职责
* 我们可能需要一个默认的机制, 这应该是接收者的责任
...
* 在这种清况下，如何将“行为请求者”与“行为实现者”解耦？将—组行为抽象为对象， 可以实现二者之间的松耦合。

#### 定义
> 将—个请求（行为）封装为一个对象，从而使你可用不同的请求对客户进行参数化；对请求排队或记录请求日志，以及支持可撤销的操作。

#### 结构
<div align=center><img src="https://i.imgur.com/QqUiLMm.png"/></div>

我们在虚基类 [Command](code/Command.cpp) 中只有一个纯虚函数 `execute`, 然后不同的实现都去继承它,具体的实现中可以有丰富的参数信息, 甚至可以有一个类似于宏的实现. 对象的使用就很灵活了. 

#### 要点总结
* Command模式的根本目的在于将“行为请求者”与“行为实现者”解耦，在面向对象语言中，常见的实现手段是“**将行为抽象为对象**”。
* 实现Command接口的具体命令对象ConcreteCommand有时候根据需要可能会保存一些额外的状态信息。通过使用Composite模式，可以将多个“命令”封装为一个“复合命令” MacroCommand 。
* Command 模式与C+＋ 中的函数对象有些类似,都是**将行为对象化**。但两者定义行为接口的规范有所区别： Command 以面向对象中的“接口－实现”来定义行为接口规范，更严格，但有性能损失； C＋＋函数对象以函数签名来定义行为接口规范，更灵活，性能更高。

### 访问器模式
#### 动机
* 在软件构建过程中，由千需求的改变，某些类层次结构中常常需要增加新的行为（从基类到所有的子类都要添加），如果直接在基类中做这样的更改，将会给子类带来很繁重的变更负担，甚至破坏原有设计。
* 如何在不更改类层次结构的前提下，在运行时(扩展)根据需要透明地为类层次结构上的各个类动态添加新的操作

#### 定义
我们这里有个前提是, 知道未来需要添加操作, 但是不知道要添加多少,添加什么. 这时候需要[重新设计](code/ElementRefactoringWithVisitor.cpp),Element 类中放一个虚函数`accept()`, 其参数是 visitor的引用, 起子类 ElementA 中的就调用 `visitor.visitorElementA(*this)`. 如果需要更改, 就实现 visitor 的一个子类 VisitorA, 在其中实现 `visitorElementA`. `someElem.accept(someVisitor)` 中就有一种被称为**第二次多态辨析的过程**:
* 第一次是确定 someElem 是什么运行时类型, 确定调用谁的 accept
* 第二次是调用 accept 的时候, 根据参数的类型确认调用的是哪个 visitor 的 `visiterElement()` 实现
> 表示一个作用于某对象结构中的各元素的操作。使得可以在不改变（稳定）各元素的类的前提下定义（扩展）作用千这些元素的新操作（变化）。

#### 结构
<div align=center><img src="https://i.imgur.com/1DdVMfp.png"/></div>

这个模式有个非常大的缺点是没有办法添加子类, 我们写 visitor 基类的时候**必须知道 element 有几个子类**, 然后分别去实现对不同子类的方法, 要求子类稳定其实是相当苛刻的. 

#### 要点总结
* Visitor 模式通过双重分发来实现在不更改 Element 层次结构的前提下, 在运行时透明地为类层次结构上的各个动态类添加新的操作
* 最大的缺点在于无法扩展类的层次结构, 只适用于 **Element 子类层次结构稳定, 而其中的操作却频繁面临改动**的场景
...
#### 结构
<div align=center><img src="https://i.imgur.com/cBb0517.png"/></div>

#### 要点总结
* Interpreter模式的应用场合是Interpreter模式应中的难点，只有满足“业务规则频繁变化且类似的结构不断重复出现，并且容易抽象为语法规则的问题”才适合使用Interpreter模式。
* 使用该模式可以使用 OO 的技巧方面地扩展文法
* 比较适合简单的文法表示，对于复杂的文法表示，Interperter模式会产生比较大的类层次结构，需要求助于语法分析生成器这样的标准工具。

## 总结
学习设计模式的一个核心目标是 ==管理变化,提高复用==.在八大原则的指导下, 我们常常使用重构的方式来迭代我们的设计:
* 静态 -> 动态
* 早绑定 -> 晚绑定
* 继承 -> 组合
* 编译时依赖 -> 运行时依赖
...
有的模式在今天已经不再常用: <div align=center><img src="https://i.imgur.com/6SEUEt6.png"/></div>

最终大多数情况下都是使用第三种方案, 继承和组合对象在类模型中很大程度是一样的,都带来紧耦合. 但是组合指针就有很大的不同, 指针带来很大的灵活性(可以指向 B 的子类):
<div align=center><img src="https://i.imgur.com/eYXrysP.png"/></div>

我们应该审慎地使用设计模式, 良好地设计是演化地结果:
* 代码可读性很差时
* 需求理解还很浅时
* 变化没有显现时
* 不是系统的关键依赖点

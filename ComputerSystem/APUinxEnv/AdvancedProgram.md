...
我们先在这里讨论打开文件和读写文件, UNIX 中的大多数文件 I/O 只要5个函数, `open, read, write, lseek` 和 `close`.这些哈数被成为 不带缓存的 I/O, 意味着每个read和write都调用内核中的一个系统调用。<br>
...
```
上面的 ... 表示剩下的参数数量以及类型是可变的, 第一个 oflag 可以是只读打开,只写打开, 读写打开, 只执行打开 和 只搜索打开(用于目录), 还有很多可选的项目. <br>
...
引入后面那个函数还有一个目的是避免 time-of-check-to-time-of-use (TOCTTOU) 错误, 其基本思想是 **如果有两个基文件的函数调用，其中第二个调用依赖于第一个调用的结果，那么程序是脆弱的**. <br>

...
#### 实时调度
实时任务具有非常高的的时效性， 必须对请求和事件立即做出反应，因此会有一个**明确地截止时间**，超出截止时间造成后果的严重性可之分成**硬实时任务**和**软实时任务**两类. 同时根据触发时间可以分成**周期任务**、**偶发任务**和**非周期任务**三类。  
实时操作系统最重要的特点是**确定性**， 完成时间需要明确地上界。  
**速率单调**（RM）是一个用于调度周期任务的静态优先级实时调度策略，速率指的是优先级的倒数。RM 中任务的周期越短， 那么其优先级就越高，RM策略还支持抢占。在 $U\leq 1$ 不一定能够满足所有的截止时间要求。  
**最短截止时间优先**（EDF）：在 U>1 的时候，该策略会因为一个任务错过截止时间而导致大量后续任务级联错过截止时间（多米诺效应）。

...
#### 虚拟化 CPU
从结果上看, 在硬件的帮助下, 操作系统负责提供一种假象, **系统拥有非常多的虚拟CPU**.   
...
#### 受限制的操作
在 CUP 上运行会带来一个问题, 如果进程希望执行某种受限操作(如 IO/申请内存), 应该如何做?  
**硬件通过提供不同的执行模式来协助操作系统**, 在用户模式下, 应用程序不能完全访问硬件资源, 在内核模式下, 操作系统可以访问机器的全部资源. 还提供了陷入(trap)内核和从陷进返回(return-from-trap)到用户模式程序的说明, 以及一些指令, 让操作系统告诉硬件陷阱表(trap table) 在内存中的位置.  
...
时间片太短是有问题的: 突然上下文切换的成本将影响整体性能(不仅来自于保存和恢复少量寄存器的操作, **在 CPU 高速缓存/TLB/分支预测和其他片上硬件中建立的大量状态刷新和引入**), 系统设计者需要权衡时间片的长度, 使其足够长以便摊销(amortize)上下文切换的成本.  
...
### Chap 13: 抽象 -- 地址空间
再去的操作系统是一组函数(实际上是一个库), 几乎没有抽象. 但是很快, 分时系统诞生了, 内存有过一种实现时分共享的办法: 让一个进程单独占用全部内存一段时间, 然后停止它, 将状态信息保存在磁盘上, 加载其他进程的状态信息. 这种方法有很多问题:
* 太慢了: 保存和恢复寄存器状态信息相对较快, 将全部内存信息写入磁盘就太慢了
* 多个程序驻留在内存中,保护称为重要问题, 人们不希望一个进程可以读取其他进程的内存信息, 更别说修改.

#### 地址空间
为了解决这些烦人的问题, 操作系统提供了一个物理内存的抽象(被称为地址空间).
<div align=center><img src="https://raw.githubusercontent.com/Haitau1996/picgo-hosting/master/img/20210520155642.png"/></div>

#### 目标
VM 系统有很多目标:
* 透明性: 程序不应该感知内存被虚拟化的现实
* 效率: 应该追求尽量高效
* 保护

### Chap 14: 内存操作 API
#### 内存类型
第一种被称为栈内存, 申请和释放是编译器隐式管理的,也被称为栈内存.   
对于长期内存的需求, 需要堆内存, 申请和释放都需要又程序员显示完成. 

#### `malloc` 调用
调用时候需要先包含头文件 `<stdlib.h>`, 传入申请的内存大小, 成功就返回一个指向新申请的空间的指针, 失败则返回 `NULL`.  
这通常搭配 sizeof, 有几个细节需要注意:
* 对于array/pointer,  得到的结果是不一样的
* 对于字符串, 最好使用 `malloc(strlen(s)+1)`, 而不是使用`malloc(sizeof(s))`

此外我们常用强制转换 malloc 的结果, 将它从 `void*` 转成我们希望的指针类型, 但这不是必须的.

#### 常见的错误
1. 忘记分配内存, 很多例程在调用时希望你为它们分配内存,例如 `strcpy(dest, src)`
   ```C++
   char *src = "hello"; 
   char *dst; // oops! unallocated 
   strcpy(dst, src); // segfault fault and die
   ```
   上面的例子中可以先给 det malloc 或者用 `strdup()`
2. 没有分配足够的内存: 这被称为缓存区溢出(buffer overflow), 结果是未定义的
3. 忘记初始化分配的内存直接使用: 结果是未定义的
4. 忘记释放内存:在长时间的应用程序或者系统中是一个巨大问题, 导致内存不足需要重新启动
5. 在使用结束之前释放内存: 被称为悬挂指针
6. 反复释放内存: 结果是未定义的,通常会使得程序崩溃
7. 错误调用 `free`(如对非堆内存地址调用)

对 4 做一个简要的补充, 操作系统内其实是存在两级内存管理. 第一级由操作系统管理, 进程运行时将内存交给进程, 进程退出时回收. 第二级是在进程中通过调用 `malloc` 和 `free` 管理, 即使没有调用`free`, 操作系统也会在程序结束运行时, 回收所有的内存.  
**malloc 库管理虚拟地址空间内的空间, 是建立在一些系统调用之上的**, 如 `brk` 用与改变程序分段的位置. 我们不应该直接调用这些系统调用.  
内存分配库还支持 `calloc()`(在返回地址前将其置0) 和 `realloc()` 穿件更大的内存区域, 将旧区域复制到其中, 并返回新区域的地址.
...
<div align=center><img src="https://raw.githubusercontent.com/Haitau1996/picgo-hosting/master/img/20210521151602.png"/></div>
